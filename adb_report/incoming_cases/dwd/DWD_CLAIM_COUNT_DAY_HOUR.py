...
WITH tmp_11 AS (
    ...
    and substr(T_CRT_TIME, 1, 10) = '{formatted_date}'
),
a1 AS (
    -- 第一个子查询部分
    SELECT
...
a4 AS (
    ...
    WHERE c.delete_flag = '0'
      AND substr(c.create_time, 1, 10) = '{formatted_date}'
...
a5 AS (
    ...
    WHERE pr.insure_company_channel NOT IN ('CP01', 'YX01', 'common')
      ...
      AND receiver = 'I'
      AND substr(back_time, 1, 10) = '{formatted_date}'
    ...
    WHERE
        ...
        AND substr(fr.CREATE_TIME, 1, 10) = '{formatted_date}'
...
a11 AS (
        ...
        (
	        ...
	        where substr(q.gmt_created,1,10) = '{formatted_date}'
...
def truncate_table(table_name='CLAIM_DWD.DWD_CLAIM_COUNT_DAY_HOUR'):
    with DatabaseConnection() as conn:
        truncate_sql = f"delete  from  {table_name}  where GMT_CREATED = '{formatted_date}'  "
