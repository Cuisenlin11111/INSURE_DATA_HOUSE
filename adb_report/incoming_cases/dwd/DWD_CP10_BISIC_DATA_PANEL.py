...
WITH a1 AS (
    SELECT
        ...
        a.department_code as insure_company_channel,
    ...
    WHERE a.claim_source in ( '2','4')
      and a.insure_company_channel='CP10' AND a.department_code  in ('大连分公司','苏州分公司')
      AND
          NOT EXISTS (
        SELECT 1
    ...
    GROUP BY a.department_code, SUBSTR(c.create_time, 1, 10)
),
a2 AS (
    SELECT
...
a3 AS (
    ...
    JOIN claim_ods.claim c ON a.accept_num = c.acceptance_no AND c.delete_flag = '0'
    WHERE
        a.insure_company_channel='CP10' AND a.department_code  in ('大连分公司','苏州分公司') and
        EXISTS (
        SELECT 1
        FROM claim_ods.bill b
        WHERE b.claim_id = c.id AND b.delete_flag = '0' AND b.treatment_date IS NULL
...
a4 AS (
    ...
    left join claim_ods.accept_list_record a on a.accept_num = c.acceptance_no AND a.DEL_FLAG = '0'
    WHERE pr.is_deleted = 'N'
    and  a.insure_company_channel='CP10' AND a.department_code  in ('大连分公司','苏州分公司')
      AND (pr.postback_way = 'W' OR pr.postback_way IS NULL)
    GROUP BY a.department_code, SUBSTR(pr.gmt_created, 1, 10)
),
a5 AS (
    SELECT
        a.department_code as insure_company_channel,
    ...
    INNER JOIN claim_ods.postback_record pr ON c.claim_no = pr.app_no AND pr.is_deleted = 'N'
    left join claim_ods.accept_list_record a on a.accept_num = c.acceptance_no AND a.DEL_FLAG = '0'
    WHERE pr.is_deleted = 'N' AND pr.postback_way = 'H' and  a.insure_company_channel='CP10' AND a.department_code  in ('大连分公司','苏州分公司')
    GROUP BY a.department_code, SUBSTR(pr.gmt_created, 1, 10)
)
SELECT
