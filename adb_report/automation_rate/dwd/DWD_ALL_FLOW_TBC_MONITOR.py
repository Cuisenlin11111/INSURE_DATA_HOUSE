...
sql_query = f"""
insert into CLAIM_DWD.DWD_ALL_FLOW_TBC_MONITOR
...
with a1 as
     (
     -- 在原有的筛选条件基础上添加时间条件，筛选最近100天的数据
     select '太保财全渠道' insure_company_channel,
             count(*) in_claim,
       ...
       where a.DEL_FLAG = '0'
         and c.delete_flag = '0'
         and c.insure_company_channel in('CP10','CP08','CP02')
    ...
    a12 as
     (select gmt_created,
        ...
        from (select count(id) 配置总数,
               ...
               where t.insure_company_channel in('CP10','CP08','CP02')
               and  substr(t.gmt_created, 1,10) >= (CURRENT_DATE - INTERVAL '100' DAY) -- 在子查询中添加时间筛选条件
               group by substr(t.gmt_created, 1,10))
       group by gmt_created),
    a13 as
     (
      -- 泰康全渠道 审核自动化率(任务) 单独如下计算，添加时间条件
      select a2.datestr,
              case
      ...
      select d1.comm_date,
        ...
        from (select count(DISTINCT cat.C_CLAIM_CASE_NO) zdshal,
                ...
                WHERE pr.back_status in ('2', '21')
                  ...
                  and pr.insure_company_channel in('CP10','CP08','CP02')
        ...
        left join (select substr(gmt_created, 1,10) comm_date,
                    ...
                    where pr.back_status in ('2', '21')
                      and pr.is_deleted = 'N'
                      and (pr.postback_way = 'W' or pr.postback_way is null)
                      and insure_company_channel in('CP10','CP08','CP02')
     ...
     (
      ...
      select substr(cat.T_CRT_TM, 1,10) comm_date,
        ...
        join claim_ods.claim c
          on a.accept_num = c.acceptance_no
         and c.delete_flag = '0'
         and a.insure_company_channel in('CP10','CP08','CP02')
...
"""
def truncate_table(table_name='CLAIM_DWD.DWD_ALL_FLOW_TBC_MONITOR'):
