...
sql_query = f"""
  ...
  -- @author: 01
  -- @version: 1.0.0
INSERT INTO CLAIM_DM.ADM_PRO_ROFIT_M_NEW
  (    insurance_company_channel ,
   ...
   Operation_Cost_Ratio,
    ...
    t2 as  (select entry_merchant,
       ...
       expense 录入成本,
       avg_expense 录入均价
from   CLAIM_DWS.DWS_INPUT_COST_M   WHERE entry_merchant='合计'),
    t3 as  (select DT_MONTH AS 年月,
       case when  CHANNEL_VALUE='泰康养老甘肃分公司'  then '泰康养老甘肃-半流程'  else  CHANNEL_VALUE  end  AS 大渠道,
...
from CLAIM_DIM.DIM_MANPOWER_CONFIG)
     select
       ...
       cast(t1.除税收入-coalesce(t2.录入成本,0)- t3.项目作业人力成本 as decimal(10,2)) 毛利润,
       cast((t1.除税收入-coalesce(t2.录入成本,0)- t3.项目作业人力成本 )/t1.除税收入 as decimal(10,4))  毛利润率,
...
#        cast((t1.除税收入 - coalesce(t2.录入成本,0) - t1.营销成本 - t3.项目作业人力成本 - t3.技术人力成本)/t1.除税收入  as decimal(10,4)) 单纯净利润率,
       ...
       cast(t3.项目作业人力成本/t1.案件收入  as decimal(10,2))   项目作业成本占比,
       cast((t3.项目作业人力成本 + t3.技术人力成本 +coalesce(t2.录入成本,0)  ) /t1.案件量 + t4.HLP_MANAGE_CLAIM + t4.YSJ_MANAGE_CLAIM +t1.营销成本/t1.案件量  as decimal(10,2))  总成本案件,
       ...
       cast(coalesce(t5.expense,0)/t1.案件收入 as decimal(10,4))   施博录入成本占比,
       ...
       cast(coalesce(t6.expense,0)/t1.案件收入 as decimal(10,4))  成都视觉录入成本占比,
       ...
       cast(coalesce(t7.expense ,0)/t1.案件收入 as decimal(10,4))  广纳录入成本占比,
       ...
       cast(coalesce(t8.expense ,0)/t1.案件收入 as decimal(10,4))  因朔桔录入成本占比,
       cast(coalesce(t8.avg_expense ,0) as decimal(10,2)) 因朔桔录入均价,
       coalesce(t8.IN_CLAIM_NUM ,0) 因朔桔录入案件量,
       cast(coalesce(t8.expense ,0) as decimal(10,2)) 因朔桔录入成本,
       REPLACE(CURDATE(),'-','')
     from  t1
    left join  t2
...
"""
def truncate_table(table_name='CLAIM_DM.ADM_PRO_ROFIT_M_NEW'):
