...
INSERT INTO  CLAIM_DWD.DWD_TK12_TOTAL
WITH tt AS (
        SELECT DISTINCT
        ...
        coalesce(group_concat(DISTINCT inv.C_RESPONSE_DESC), '') AS 责任描述,
        '结案' AS is_back
    ...
    WHERE pr.INSURE_COMPANY_CHANNEL = 'TK12'
...
ff AS (
    SELECT
        时间,
        '泰康养老福建分公司' AS INSURE_COMPANY_CHANNEL,
        责任描述,
        回传方式,
        is_back,
        受理编号,
        ...
        CASE
            ...
            WHEN 责任描述 LIKE '%住院%'  THEN 5.4
            WHEN 责任描述 NOT LIKE '%住院%'  THEN 4.73
...
def truncate_table(table_name='CLAIM_DWD.DWD_TK12_TOTAL'):
