...
WITH current_data AS (
    SELECT INSURE_COMPANY_CHANNEL, DRHCAL*2 AS DRHCAL
...
-- 创建临时表last_week_data，存储上一周符合条件的数据，并计算平均DRHCAL
last_week_data AS (
    SELECT INSURE_COMPANY_CHANNEL, AVG(DRHCAL) AS avg_DRHCAL_last_week
    FROM (
        SELECT INSURE_COMPANY_CHANNEL, DRHCAL
        FROM CLAIM_DWD.DWD_CLAIM_COUNT_DAY
...
WHERE
    ((cd.DRHCAL - lw.avg_DRHCAL_last_week) / lw.avg_DRHCAL_last_week) * 100 > 50
    OR ((cd.DRHCAL - lw.avg_DRHCAL_last_week) / lw.avg_DRHCAL_last_week) * 100 < -50
;