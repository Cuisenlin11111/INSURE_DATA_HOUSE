...
WITH current_data AS (
    SELECT INSURE_COMPANY_CHANNEL, CANCEL_VOL*2 AS CANCEL_VOL
...
-- 创建临时表last_week_data，存储上一周符合条件的数据，并计算平均CANCEL_VOL
last_week_data AS (
    SELECT INSURE_COMPANY_CHANNEL, AVG(CANCEL_VOL) AS avg_CANCEL_VOL_last_week
    FROM (
        SELECT INSURE_COMPANY_CHANNEL, CANCEL_VOL
        FROM CLAIM_DWD.DWD_CLAIM_COUNT_DAY
...
WHERE
    ((cd.CANCEL_VOL - lw.avg_CANCEL_VOL_last_week) / lw.avg_CANCEL_VOL_last_week) * 100 > 50
    OR ((cd.CANCEL_VOL - lw.avg_CANCEL_VOL_last_week) / lw.avg_CANCEL_VOL_last_week) * 100 < -50
;