...
WITH current_data AS (
    SELECT INSURE_COMPANY_CHANNEL, LRL*2 AS LRL
...
-- 创建临时表last_week_data，存储上一周符合条件的数据，并计算平均LRL
last_week_data AS (
    SELECT INSURE_COMPANY_CHANNEL, AVG(LRL) AS avg_LRL_last_week
    FROM (
        SELECT INSURE_COMPANY_CHANNEL, LRL
        FROM CLAIM_DWD.DWD_CLAIM_COUNT_DAY
...
WHERE
    ((cd.LRL - lw.avg_LRL_last_week) / lw.avg_LRL_last_week) * 100 > 50
    OR ((cd.LRL - lw.avg_LRL_last_week) / lw.avg_LRL_last_week) * 100 < -50
;