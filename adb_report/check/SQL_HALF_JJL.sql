...
WITH current_data AS (
    SELECT INSURE_COMPANY_CHANNEL, JJL*2 AS JJL
...
-- 创建临时表last_week_data，存储上一周符合条件的数据，并计算平均JJL
last_week_data AS (
    SELECT INSURE_COMPANY_CHANNEL, AVG(JJL) AS avg_jjl_last_week
    FROM (
        SELECT INSURE_COMPANY_CHANNEL, JJL
        FROM CLAIM_DWD.DWD_CLAIM_COUNT_DAY
...
WHERE
    ((cd.JJL - lw.avg_jjl_last_week) / lw.avg_jjl_last_week) * 100 > 50
    OR ((cd.JJL - lw.avg_jjl_last_week) / lw.avg_jjl_last_week) * 100 < -50
;