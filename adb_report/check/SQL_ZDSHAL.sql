...
WITH current_data AS (
    SELECT INSURE_COMPANY_CHANNEL, SHZDHL
...
-- 创建临时表last_week_data，存储上一周符合条件的数据，并计算平均SHZDHL
last_week_data AS (
    SELECT INSURE_COMPANY_CHANNEL, AVG(SHZDHL) AS avg_SHZDHL_last_week
    FROM (
        SELECT INSURE_COMPANY_CHANNEL, SHZDHL
        FROM CLAIM_DWD.DWD_CLAIM_COUNT_DAY
...
WHERE
    ((cd.SHZDHL - lw.avg_SHZDHL_last_week) / lw.avg_SHZDHL_last_week) * 100 > 50
    OR ((cd.SHZDHL - lw.avg_SHZDHL_last_week) / lw.avg_SHZDHL_last_week) * 100 < -50
;
