...
WHERE
    substr(t.create_time,1,7) = '{last_month}'
    AND t.deal_type = '3'
    AND t.deleted = '0'
...
WHERE
    ...
    AND substr(cl.create_time,1,7) = '{last_month}'
    AND cl.deleted = '0'
GROUP BY
...
WHERE
    t.node = '预处理'
    AND t.handle = '预处理'
    AND substr(t.create_time,1,7) = '{last_month}'
    AND t.deleted = '0'
GROUP BY
...
WHERE
     b.node = '录入'   AND b.handle = '账核诊录入'
  and substr(b.create_time,1,7) = '{last_month}'
and a.deleted = 0
"""
...
LEFT JOIN
    inphile_ods.account c ON b.user_id = c.account
WHERE
    substr(b.create_time,1,7) = '{last_month}'
    and  b.node IN ('复核', '质检')
...
if __name__ == "__main__":
    excel_filename1 = "/opt/adb_report/report_claim/output/" + f"因朔桔录入电票手动{last_month_formatted}任务量统计.xlsx"
    excel_filename2 = "/opt/adb_report/report_claim/output/" + f"因朔桔录入特殊件{last_month_formatted}任务量统计.xlsx"
    excel_filename3 = "/opt/adb_report/report_claim/output/" + f"因朔桔录入预处理{last_month_formatted}任务量统计.xlsx"
    excel_filename4 = "/opt/adb_report/report_claim/output/" + f"因朔桔录入{last_month_formatted}任务量统计.xlsx"
    excel_filename5 = "/opt/adb_report/report_claim/output/" + f"因朔桔复核{last_month_formatted}任务量统计.xlsx"
    excel_filename6 = "/opt/adb_report/report_claim/output/" + f"因朔桔手动回传{last_month_formatted}任务量统计.xlsx"
    ...
    send_weekly_report_email(last_month_formatted, excel_filename1, excel_filename2, excel_filename3, excel_filename4, excel_filename5, excel_filename6)
